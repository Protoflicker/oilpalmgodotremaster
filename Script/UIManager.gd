...
func _ready():
	...
	if capture_label:
		capture_label.text = "Tertangkap: 0 kali"
...
func _update_extraction_hint():
	...
	extraction_hint.text = "Mobil pickup: %d m   (%s)" % [int(dist), arrow]
...
func update_quota_display(current_kg: int, target_kg: int):
	...
	if not quota_label:
		return
	if target_kg <= 0:
		quota_label.text = "Mode Latihan (tanpa kuota)"
		quota_label.modulate = Color.WHITE
	else:
		quota_label.text = "Kuota: %d / %d kg" % [current_kg, target_kg]
...
func update_ui_from_player():
	...
	if ripe_label:
		...
		ripe_label.text = "Buah matang: %d dibawa, %d kg disetor" % [carried_ripe, delivered_ripe_kg]
	if unripe_label:
...
func show_delivery_notification(total_kg: int):
	show_notification("%d kg buah matang berhasil disetor!" % total_kg)
...
func show_round_end_notification(final_score: int, score_details: Dictionary):
	...
	if final_score_label:
		final_score_label.text = "BERHASIL KABUR!  +Rp %s" % _format_currency(final_score)
	...
	if details_label:
		...
		var text = "RINCIAN:\n"
		text += "+ Buah disetor : %d kg = Rp %s\n" % [ripe_kg, _format_currency(ripe_income)]
		text += "+ Bonus waktu  : %d dtk = Rp %s\n" % [rem, _format_currency(time_bonus)]
		text += "Total uang     : Rp %s" % _format_currency(GameState.total_money)
...
func _on_caught_respawn(reason: String):
	capture_count_ui += 1
	if capture_label:
		capture_label.text = "Tertangkap: %d kali" % capture_count_ui
	show_notification("Tertangkap! " + reason)
